...
class robotNode(Node):
    ...
    def getGraspPoint(self):
        ...
        fill in here
        '''
        gx, gy, gz =X-0.0325, Y-0.05949, Z-0.1
    ...
    def RobotTask(self, grasp_pos):
        ...
        self.sendCommand("movel", 'tool', [gx, gy,0, 0, 0, 0])

        # 3) move down to grasp point (z)
        self.sendCommand("movel", 'tool', [0, 0, gz, 0, 0, 0])

        # 4) gripper close
        self.sendCommand("gripper", "close")
        
        # 5) pick up toothbrush
        self.sendCommand("movel", 'tool', [0, 0, -0.3, 0, 0, 0])

        # 6) place toothbrush
        self.sendCommand("movej", 'abs', [-98.05, -51.99, -78.5, -95.07, 141.71, 75.75, 9.96])     
        time.sleep(3)

        self.sendCommand("movel", 'base_rel', [0, 0, -0.05, 0, 0, 0])
        self.sendCommand("gripper", "open")
