...
class robotNode(Node):
    def __init__(self):
        ...
        body_offsets = [0.0, 0.117, 0.12 + 0.115]  # 누적 y 값

        self.body_slots = []
        for i, dy in enumerate(body_offsets):
            p = case1.copy()
            p[1] += dy
    ...
    def auto_match_task(self):
        ...
        1) 안전 자세로 이동
        2) 케이스 3개 크기 측정
        3) 뚜껑 5개 크기 측정 (4→3→2→1→0 순서)
        4) 크기 기반으로 최적 매칭
        5) 각 pair에 대해 assemble_lid_to_body 호출
        """
        # 1) 안전 자세
        ...
        # 2) 케이스 측정 (0,1,2 순서)
        self.get_logger().info("=== Measure bodies (cases) ===")
        body_sizes = []
        for idx, pose in enumerate(self.body_slots):
            label = f"body_{idx}"
            size = self.measure_slot(pose, label)
            body_sizes.append(size)

        # 케이스 보정 (측정값이 2mm 크게 나와서 -0.002m)
        CASE_OFFSET = -0.002
        for i, s in enumerate(body_sizes):
            if not np.isnan(s):
                corrected = s + CASE_OFFSET
                self.get_logger().info(
                    f"[CALIB] body_{i}: raw={s:.4f} m -> corrected={corrected:.4f} m"
                )
                body_sizes[i] = corrected

        # 3) 뚜껑 측정 (4→3→2→1→0 순서로 스캔)
        self.get_logger().info("=== Measure lids (reverse order 4→0) ===")
        lid_sizes = [np.nan] * len(self.lid_slots)

        for idx in reversed(range(len(self.lid_slots))):  # 4,3,2,1,0
            pose = self.lid_slots[idx]
            label = f"lid_{idx}"
            size = self.measure_slot(pose, label)
            lid_sizes[idx] = size

        self.get_logger().info(f"body_sizes = {body_sizes}")
        self.get_logger().info(f"lid_sizes  = {lid_sizes}")

        # 4) 단순 greedy 매칭: body마다 가장 가까운 lid를 하나씩 매칭
        matched_pairs = []
        used_lids = set()

        for b_idx, b_size in enumerate(body_sizes):
            if np.isnan(b_size):
                self.get_logger().warn(f"body {b_idx} size NaN, skip")
                continue

            best_l = None
            best_diff = None
            for l_idx, l_size in enumerate(lid_sizes):
                if l_idx in used_lids:
                    continue
                if np.isnan(l_size):
                    continue
                diff = abs(b_size - l_size)
                if (best_diff is None) or (diff < best_diff):
                    best_diff = diff
                    best_l = l_idx

            if best_l is None:
                self.get_logger().warn(f"No available lid for body {b_idx}")
            else:
                self.get_logger().info(
                    f"[MATCH] body {b_idx} (size={b_size:.4f} m) "
                    f"<- lid {best_l} (size={lid_sizes[best_l]:.4f} m), diff={best_diff:.4f} m"
                )
                matched_pairs.append((b_idx, best_l))
                used_lids.add(best_l)

        self.get_logger().info(f"Matched pairs: {matched_pairs}")

        # 5) 매칭된 pair마다 조립 동작
        for (b_idx, l_idx) in matched_pairs:
            self.assemble_lid_to_body(b_idx, l_idx)

        self.get_logger().info("=== auto_match_task finished ===")
    ...
    def assemble_lid_to_body(self, body_idx, lid_idx):
        ...
        z_lift = 0.10
        x_offset = 0.003
        y_offset = -0.006    # ← body 쪽으로 갈 때 y를 이만큼 더 밀어줌
